...
def test_get_items_success(test_client: TestClient):
    ...
    response = test_client.get("/api/v1/admin/items", headers=headers)
...
def test_basic_register(test_client: TestClient, test_faker: Faker):
    ...
    headers = get_authorization_header(test_client,"poncebernard123", "loremIpsum123")
    response = test_client.post("/api/v1/admin/items", json=pay_load, headers=headers)
    assert response.status_code == 201, "Failed to Create a new Record/Data."
    created_data = response.json()
    response = test_client.get(f"/api/v1/admin/items/{created_data.get('id')}", headers=headers)
...
def test_create_and_update(test_client: TestClient, test_faker: Faker):
    ...
    response = test_client.put(f"/api/v1/admin/items/{created_data.get('id')}", json=new_data, headers=headers)
...
def test_create_and_delete(test_client: TestClient, test_faker: Faker):
    ...
    response = test_client.delete(f"/api/v1/admin/items/{created_data.get('id')}", headers=headers)
...
def test_register_with_missing_column1(test_client: TestClient, test_faker: Faker):
    ...
    response1 = test_client.post("/api/v1/admin/items", json=pay_load1, headers=headers)
    ...
    response2 = test_client.post("/api/v1/admin/items", json=pay_load2, headers=headers)
